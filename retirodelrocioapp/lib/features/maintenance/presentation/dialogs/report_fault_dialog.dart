...
class _ReportFaultDialogState extends ConsumerState<ReportFaultDialog> {
    ...
    try {
      final order = await ref
          ...
          .createWorkOrder(
            roomUnitId: _room?.id,
            assetId: _asset?.id,
            assetLabel: _asset == null && _assetController.text.trim().isNotEmpty
      ...
      for (final file in _attachments) {
        try {
          await ref.read(maintenanceActionsProvider(widget.token)).uploadAttachment(order.id, file.path);
        } catch (_) {
            ...
            child: Column(
              ...
              children: [
                ...
                _field(
                  ...
                  label: 'Or a new asset name, e.g. "Lobby generator" (optional)',
                  enabled: _asset == null,
                ),
                const SizedBox(height: 12),
                _priorityPicker(),
  ...
  Widget _assetDropdown(List<Asset> assets) {
    return DropdownButtonFormField<Asset?>(
      ...
      items: [
        ...
        for (final asset in assets) DropdownMenuItem<Asset?>(value: asset, child: Text(asset.name)),
      ],
  ...
  Widget _attachmentChip(int index) {
    ...
    final isVideo = ['.mp4', '.mov', '.webm'].any((ext) => file.path.toLowerCase().endsWith(ext));
    return Container(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(isVideo ? Icons.videocam_rounded : Icons.image_rounded, size: 14, color: Colors.white70),
          const SizedBox(width: 6),
          Text(isVideo ? 'Video' : 'Photo', style: AppTypography.style(color: Colors.white70, fontSize: 12)),
