...
final maintenanceAssetsProvider = FutureProvider.family<List<Asset>, String>((ref, token) async {
  final repo = ref.watch(maintenanceRepositoryProvider);
...
final maintenanceAssetDetailProvider = FutureProvider.family<Asset?, (String, int)>((ref, args) async {
  final repo = ref.watch(maintenanceRepositoryProvider);
...
/// refresh.
final maintenanceWorkOrderDetailProvider = FutureProvider.family<WorkOrder?, (String, int)>((ref, args) async {
  final repo = ref.watch(maintenanceRepositoryProvider);
...
final maintenancePartsRequestsProvider = FutureProvider.family<List<PartsRequest>, String>((ref, token) async {
  final repo = ref.watch(maintenanceRepositoryProvider);
...
/// technician.
final maintenanceTechniciansProvider = FutureProvider.family<List<Technician>, String>((ref, token) async {
  final repo = ref.watch(maintenanceRepositoryProvider);
...
final maintenanceNotificationsRealtimeProvider = FutureProvider.family<void, String>((ref, token) async {
  final config = (await ref.watch(appConfigProvider.future)).realtime;
  if (config == null) {
    debugPrint('maintenanceNotificationsRealtime: no broadcaster configured — polling only.');
    return;
  }

  final channel = SosChannel(config: config, channel: 'maintenance', events: const {});
  channel.connect(
    onChanged: () {},
    onNotification: () {
...
/// screen.
final maintenanceSlaBreachChimeProvider = Provider.family<void, String>((ref, token) {
  ...
  ref.listen<AsyncValue<List<WorkOrder>>>(
    ...
    (previous, next) {
      ...
      final breachedNow = list.where((o) => o.slaBreached).map((o) => o.id).toSet();
      final knownBreached = previouslyBreached;
      if (knownBreached != null) {
        final newlyBreached = list.where((o) => o.slaBreached && !knownBreached.contains(o.id));
...
class MaintenanceActions {
  ...
  Future<Asset> createAsset({
    ...
    final asset = await _ref.read(maintenanceRepositoryProvider).createAsset(
      _token,
      name: name,
      category: category,
      roomUnitId: roomUnitId,
  ...
  Future<Asset> markAssetServiced(int id) async {
    final asset = await _ref.read(maintenanceRepositoryProvider).markAssetServiced(_token, id);
    _ref.invalidate(maintenanceAssetsProvider(_token));
  ...
  Future<WorkOrder> uploadAttachment(int id, String filePath) async {
    final order = await _ref.read(maintenanceRepositoryProvider).uploadAttachment(_token, id, filePath);
    _ref.invalidate(maintenanceWorkOrderDetailProvider((_token, id)));
    return order;
  }

  Future<PartsRequest> createPartsRequest(int workOrderId, {required String partName, int quantity = 1, String? note}) async {
    final request = await _ref
        .read(maintenanceRepositoryProvider)
        .createPartsRequest(_token, workOrderId, partName: partName, quantity: quantity, note: note);
    _ref.invalidate(maintenancePartsRequestsProvider(_token));
    return request;
  }

  Future<PartsRequest> fulfillPartsRequest(int id) async {
    final request = await _ref.read(maintenanceRepositoryProvider).fulfillPartsRequest(_token, id);
    _ref.invalidate(maintenancePartsRequestsProvider(_token));
    return request;
  }

  Future<PartsRequest> denyPartsRequest(int id) async {
    final request = await _ref.read(maintenanceRepositoryProvider).denyPartsRequest(_token, id);
    _ref.invalidate(maintenancePartsRequestsProvider(_token));
    return request;
  }

  Future<WorkOrder> escalate(int id) async {
    final order = await _ref.read(maintenanceRepositoryProvider).escalateWorkOrder(_token, id);
    _refresh(id);
    return order;
  }

  Future<WorkOrder> updateStatus(int id, String status) async {
    final order = await _ref.read(maintenanceRepositoryProvider).updateWorkOrderStatus(_token, id, status);
    _refresh(id);
    return order;
  }

  Future<WorkOrder> assign(int id, int technicianId) async {
    final order = await _ref.read(maintenanceRepositoryProvider).assignWorkOrder(_token, id, technicianId);
    _refresh(id);
    return order;
  }
  ...
  void _refresh([int? id]) {
    ...
    if (id != null) _ref.invalidate(maintenanceWorkOrderDetailProvider((_token, id)));
  }
}
