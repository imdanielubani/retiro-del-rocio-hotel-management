...
/// Maintenance's notification feed (`GET /maintenance/notifications`): an
/// unread count, category filter chips and the notification list — a new
/// arrival is pushed live over the hotel-wide `maintenance` realtime
/// channel; `maintenanceNotificationChimeProvider` (watched by every
/// maintenance screen) rings the chime and lights the bell's badge — this
/// screen just renders the feed.
class MaintenanceNotificationScreen extends ConsumerStatefulWidget {
  const MaintenanceNotificationScreen({
    super.key,
    required this.session,
    this.current = MaintenanceNavItem.dashboard,
  });

  final StaffSession session;

  /// The maintenance screen the technician was on before opening the bell.
...
class _MaintenanceNotificationScreenState extends ConsumerState<MaintenanceNotificationScreen> {
  ...
  List<MaintenanceNotification> _visible(List<MaintenanceNotification> all) {
    if (_filter == _Filter.all) return all;
    if (_filter == _Filter.unread) return all.where((n) => !n.read).toList();
    final category = _filter as MaintenanceNotificationCategory;
  ...
  Future<void> _open(MaintenanceNotification n) async {
    if (n.read) return;
    ...
    return SizedBox(
      ...
      child: ListView(
        ...
        children: [
          ...
          for (final category in MaintenanceNotificationCategory.values) ...[
            const SizedBox(width: 8),
  ...
  Widget _list(List<MaintenanceNotification> all) {
