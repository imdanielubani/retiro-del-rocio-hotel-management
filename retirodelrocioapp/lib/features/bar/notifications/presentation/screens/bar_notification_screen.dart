...
/// The Bar Tablet's notification feed (`GET /bar/notifications`) — same
/// visual language as [ReceptionNotificationScreen]: an unread count,
/// All/Unread filter chips, and the list. `barNotificationChimeProvider`
/// (watched by [BarShell]/[BarScaffold] everywhere) is what rings the chime
/// and lights the bell's badge — this screen just renders the feed and lets
/// the waiter act on it.
class BarNotificationScreen extends ConsumerStatefulWidget {
  const BarNotificationScreen({super.key, required this.session});

  final StaffSession session;

  @override
  ConsumerState<BarNotificationScreen> createState() =>
      _BarNotificationScreenState();
}

class _BarNotificationScreenState extends ConsumerState<BarNotificationScreen> {
  ...
  List<BarNotification> _visible(List<BarNotification> all) {
  ...
  Future<void> _markAllRead() async {
    ...
    await ref.read(barNotificationActionsProvider(_token)).markAllRead();
    if (mounted) setState(() => _markingAll = false);
  }

  Future<void> _open(BarNotification n) async {
    if (!n.read) {
      setState(() => _busyId = n.id);
      await ref.read(barNotificationActionsProvider(_token)).markRead(n.id);
      if (mounted) setState(() => _busyId = null);
    }
    if (n.diningOrderId != null && mounted) {
      await Navigator.of(context).push(
        MaterialPageRoute(
          builder: (_) => BarOrderDetailScreen(
  ...
  Widget build(BuildContext context) {
    final notificationsAsync = ref.watch(barNotificationsProvider(_token));
    final notifications = notificationsAsync.value ?? const <BarNotification>[];
    ...
    return BarScaffold(
      ...
      onChat: () => Navigator.of(context).push(
        MaterialPageRoute(
          builder: (_) => BarChatScreen(session: widget.session),
        ),
      ),
  ...
  Widget _list(List<BarNotification> all) {
    ...
    return RefreshIndicator(
      color: AppColors.gold,
      onRefresh: () async => ref.invalidate(barNotificationsProvider(_token)),
  ...
  Widget _card(BarNotification n) {
      ...
      child: Column(
        ...
        children: [
          ...
          Material(
            ...
            child: InkWell(
              onTap: () => ref.invalidate(barNotificationsProvider(_token)),
              borderRadius: BorderRadius.circular(12),
              child: Padding(
                padding: const EdgeInsets.symmetric(
                  horizontal: 24,
