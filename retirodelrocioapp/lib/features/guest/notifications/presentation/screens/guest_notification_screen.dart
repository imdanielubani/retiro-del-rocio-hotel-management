...
/// [NotificationCategory] values, so the filter needs one more state than
/// the enum alone provides.
enum _Filter { all, unread }

/// 2.7 — Notification Screen (Figma 140:3647). The guest's real hotel-updates
/// feed (`GET /tablets/notifications`): an unread count, category filter
/// chips and the notification list, each entry visually distinct while
/// unread (brighter card, gold-tinted border, a dot next to the title) and
/// settling once read. A new arrival is pushed live over the room's realtime
/// channel (see `roomRealtimeProvider`), which is what rings the chime and
/// lights the bell's badge on every guest screen — this screen just renders
/// the feed and lets the guest act on it.
class GuestNotificationScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<GuestNotificationScreen> createState() =>
      _GuestNotificationScreenState();
}

class _GuestNotificationScreenState
    extends ConsumerState<GuestNotificationScreen> {
  ...
  List<GuestNotification> _visible(List<GuestNotification> all) {
    if (_filter == _Filter.all) return all;
    if (_filter == _Filter.unread) {
      return all.where((n) => !n.read).toList();
    }
    final category = _filter as NotificationCategory;
  ...
  Future<void> _markAllRead() async {
    ...
    try {
      await ref.read(guestNotificationActionsProvider(_token)).markAllRead();
    } on GuestNotificationException catch (e) {
  ...
  Future<void> _open(GuestNotification n) async {
    if (n.read) return;
    setState(() => _busyId = n.id);
    try {
      await ref.read(guestNotificationActionsProvider(_token)).markRead(n.id);
    } on GuestNotificationException catch (e) {
      if (mounted) _toast(e.message, error: true);
    } catch (_) {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    final notificationsAsync = ref.watch(guestNotificationsProvider(_token));
  ...
  Widget _list(List<GuestNotification> all) {
    ...
    return RefreshIndicator(
      color: AppColors.gold,
      onRefresh: () async => ref.invalidate(guestNotificationsProvider(_token)),
  ...
  Widget _card(GuestNotification n) {
      ...
      child: Column(
        ...
        children: [
          ...
          Material(
            ...
            child: InkWell(
              onTap: () => ref.invalidate(guestNotificationsProvider(_token)),
              borderRadius: BorderRadius.circular(12),
              child: Padding(
                padding: const EdgeInsets.symmetric(
                  horizontal: 24,
