...
/// state than the enum alone provides.
enum _Filter { all, unread }

/// Housekeeping's real guest-request feed (`GET /housekeeping/notifications`):
/// an unread count, category filter chips and the notification list — same
/// visual language as the reception/security tablets' Notifications screen,
/// inside the housekeeping nav-rail + top-bar shell. Today the only event
/// that ever lands here is a guest raising a housekeeping request. A new
/// arrival is pushed live over the hotel-wide `housekeeping` realtime
/// channel; `housekeepingNotificationChimeProvider` (watched by every
/// housekeeping screen) is what rings the chime and lights the bell's badge
/// — this screen just renders the feed and lets the housekeeper act on it.
class HousekeepingNotificationScreen extends ConsumerStatefulWidget {
  const HousekeepingNotificationScreen({
    super.key,
    required this.session,
    this.current = HousekeepingNavItem.dashboard,
  });

  final StaffSession session;

  /// The housekeeping screen the staffer was on before opening the bell —
  /// kept so the nav rail still highlights it.
  final HousekeepingNavItem current;

  @override
  ConsumerState<HousekeepingNotificationScreen> createState() =>
      _HousekeepingNotificationScreenState();
}

class _HousekeepingNotificationScreenState
    extends ConsumerState<HousekeepingNotificationScreen> {
  ...
  List<HousekeepingNotification> _visible(List<HousekeepingNotification> all) {
    if (_filter == _Filter.all) return all;
    if (_filter == _Filter.unread) {
      return all.where((n) => !n.read).toList();
    }
    final category = _filter as HousekeepingNotificationCategory;
  ...
  Future<void> _markAllRead() async {
    ...
    try {
      await ref
          .read(housekeepingNotificationActionsProvider(_token))
          .markAllRead();
    } on HousekeepingNotificationException catch (e) {
  ...
  Future<void> _open(HousekeepingNotification n) async {
    if (n.read) return;
    ...
    return SizedBox(
      ...
      child: ListView(
        ...
        children: [
          ...
          const SizedBox(width: 8),
          _chip('Unread', _Filter.unread, badge: unreadCount),
          for (final category in HousekeepingNotificationCategory.values) ...[
            const SizedBox(width: 8),
  ...
  Widget _chip(String label, Object value, {int? badge}) {
  ...
  Widget _list(List<HousekeepingNotification> all) {
