...
/// The pre-checkout inspections reception has asked for — a room a guest is
/// due to leave has to be signed off here before reception can check them
/// out, so this is kept as its own screen rather than mixed in with the
/// general towels/amenities/DND queue.
class HousekeepingInspectionScreen extends ConsumerStatefulWidget {
  const HousekeepingInspectionScreen({super.key, required this.session});
...
class _HousekeepingInspectionScreenState
  ...
  Widget build(BuildContext context) {
    ...
    return HousekeepingScaffold(
      session: widget.session,
      active: HousekeepingNavItem.inspection,
      onNav: _onNav,
      onLogout: _logout,
