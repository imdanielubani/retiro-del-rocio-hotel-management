...
/// Reception, Housekeeping, Maintenance, or the Admin dashboard) inside the
/// same frosted shell every other security screen uses.
class SecurityChatScreen extends ConsumerStatefulWidget {
  const SecurityChatScreen({super.key, required this.session});

  final StaffSession session;

  @override
  ConsumerState<SecurityChatScreen> createState() => _SecurityChatScreenState();
}

class _SecurityChatScreenState extends ConsumerState<SecurityChatScreen> {
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 20),
                          Expanded(
                            child: StaffChatBody(session: widget.session),
                          ),
                        ],
                      ),
                    ),
