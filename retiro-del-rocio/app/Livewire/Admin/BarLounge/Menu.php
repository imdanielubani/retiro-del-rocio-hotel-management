...
class Menu extends Component
{
    ...
    public function save(): void
    {
        ...
        $data = $this->validate([
            ...
            'fCategory' => ['required', 'string', 'in:'.implode(',', $drinkSlugs)],
        ...
        $this->showForm = false;
        $this->dispatch('toast', type: 'success', message: 'Drink '.($this->editingId ? 'updated' : 'added').'.');
    }
    ...
    {
        MenuItem::drinks()->where('id', $id)->delete();
        $this->dispatch('toast', type: 'success', message: 'Drink deleted.');
    }
    ...
    public function addCategory(): void
    {
        ...
        MenuCategory::create([
            ...
            'slug' => $slug,
            'department' => 'drink',
            'sort_order' => (int) MenuCategory::drink()->max('sort_order') + 1,
        ]);
    ...
    {
        $cat = MenuCategory::drink()->findOrFail($id);
        $count = $cat->itemCount();

        if ($count > 0) {
            $this->dispatch('toast', type: 'error', message: 'Move or delete the '.$count.' '.Str::plural('drink', $count).' in "'.$cat->name.'" first.');
    ...
    {
        $base = MenuItem::query()->drinks();
        $total = (clone $base)->count();
        $active = (clone $base)->where('is_active', true)->count();

        $stats = [
            ['label' => 'Drinks on Menu', 'value' => $total, 'sub' => 'Across every drink category', 'accent' => '#f38c00'],
        ...
        $categories = MenuCategory::drink()->ordered()->get();

        $items = MenuItem::query()->drinks()
        ...
        return view('admin.bar-lounge.menu', [
            'items' => $items,
            'stats' => $stats,
            'categories' => $categories,
        ])->layout('components.admin.app', [
            'title' => 'Bar & Lounge Drinks Menu',
            'subtitle' => 'Drinks shown on the guest tablet\'s Place Order screen',
        ]);
    }
}
