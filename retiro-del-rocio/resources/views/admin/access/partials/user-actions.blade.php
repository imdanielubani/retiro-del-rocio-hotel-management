{{-- Row action menu for a user ($user). Mirrors the apartment-bookings action menu. --}}
        ...
        @if ($user->id !== auth()->id())
            <button type="button" @click="open = false" wire:click="toggleStatus({{ $user->id }})"
                ...
                @if ($user->status === 'active')
                    <svg class="size-4 text-[#6b7280]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M4.9 4.9 19 19"/></svg>
                    Suspend
                @else
            ...
            <button type="button" @click="open = false" wire:click="delete({{ $user->id }})"
                    wire:confirm="Delete {{ $user->name }}’s account? This cannot be undone."
                ...
                <svg class="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2m2 0v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/></svg>
                Delete Account
            </button>
        @endif
    </div>
