{{-- Cinema booking row action menu (fixed positioning). $b = CinemaBooking. --}}
@php
    $canUse = $b->status === 'confirmed';
    $canRecord = $b->payment_status !== 'paid' && $b->status !== 'cancelled';
    $canCancel = $b->status !== 'cancelled';
@endphp
<div x-data="{ open: false, pos: { top: '0px', left: '0px' } }" class="relative inline-block text-left">
    <button type="button" x-ref="trigger"
            @click="open = !open; if (open) { const r = $refs.trigger.getBoundingClientRect(); const w=220, h=320, g=6; let left=Math.min(Math.max(8,r.right-w),window.innerWidth-w-8); let top=(r.bottom+h+g>window.innerHeight)?(r.top-h-g):(r.bottom+g); pos={top:Math.max(8,top)+'px',left:left+'px'}; }"
    ...
    <div x-show="open" x-cloak @click.outside="open = false" x-transition.opacity.duration.100ms
        ...
        <div class="px-4 pb-2.5 pt-3 text-right">
            <p class="text-[11px] font-semibold uppercase tracking-[0.5px] text-[#9ca3af]">{{ $b->code }}</p>
            <p class="truncate text-[14px] font-bold text-[#1e1e1e]">{{ $b->customer_name ?: 'Guest' }}</p>
        </div>
        <div class="h-px bg-[#f1f1ee]"></div>

        <button type="button" @click="open = false" wire:click="viewDetails({{ $b->id }})" class="flex w-full items-center gap-3 px-4 py-2.5 text-left text-[14px] font-medium text-[#1e1e1e] transition hover:bg-[#f9fafb]">
        ...
        @if ($canUse)
            <button type="button" @click="open = false" wire:click="setStatus({{ $b->id }}, 'used')" class="flex w-full items-center gap-3 px-4 py-2.5 text-left text-[14px] font-medium text-[#16a34a] transition hover:bg-[#f0fdf4]">
                ...
                Mark Used (Checked In)
            </button>
        @endif
        @if ($canRecord)
            <button type="button" @click="open = false" wire:click="recordPayment({{ $b->id }})" class="flex w-full items-center gap-3 px-4 py-2.5 text-left text-[14px] font-medium text-[#374151] transition hover:bg-[#f9fafb]">
        ...
        <a href="{{ route('admin.cinema.receipt', $b->id) }}" target="_blank" @click="open = false" class="flex w-full items-center gap-3 px-4 py-2.5 text-left text-[14px] font-medium text-[#374151] transition hover:bg-[#f9fafb]">
        ...
        @if ($canCancel)
            <div class="h-px bg-[#f1f1ee]"></div>
            <button type="button" @click="open = false" wire:click="cancelBooking({{ $b->id }})" wire:confirm="Cancel booking {{ $b->code }}? The guest will be notified and refunded." class="flex w-full items-center gap-3 px-4 py-2.5 text-left text-[14px] font-medium text-[#dc2626] transition hover:bg-[#fef2f2]">
