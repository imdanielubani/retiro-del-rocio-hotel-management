{{-- Status-aware action menu for a smart device ($device). Mirrors
...
<div x-data="{ open: false }" class="relative flex justify-end">
    ...
    <div x-show="open" x-cloak x-transition.origin.top.right @click.outside="open = false"
        ...
        <div class="border-b border-[#f1f1ee] px-4 pb-2 pt-1 text-right">
            <p class="text-[11px] text-[#9ca3af]">{{ ucfirst($device->type) }}</p>
            <p class="truncate text-[13px] font-bold text-[#1e1e1e]">{{ $device->name }}</p>
        </div>

        <button type="button" @click="open = false" wire:click="testConnection({{ $device->id }})"
                class="flex w-full items-center gap-2.5 px-4 py-2 text-left text-[13px] text-[#374151] transition hover:bg-[#f9fafb]">
            <svg class="size-4 text-[#6b7280]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 8v4l3 2"/><circle cx="12" cy="12" r="9"/></svg>
            Test Connection
        </button>
        ...
        <button type="button" @click="open = false" wire:click="toggleActive({{ $device->id }})"
