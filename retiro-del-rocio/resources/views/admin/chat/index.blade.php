...
>
    ...
    <div class="flex h-[500px] w-[300px] shrink-0 flex-col gap-2 overflow-y-auto rounded-2xl border border-[#e5e7eb] bg-white p-3">
        @foreach ($channels as $channel)
            <button type="button" wire:click="selectContact({{ $channel['user_id'] }})"
                    @class([
                        'flex items-start gap-3 rounded-xl border p-3 text-left transition',
                        'border-[#f38c00] bg-[#fff7ed]' => $selectedContactId === $channel['user_id'],
                        'border-transparent hover:bg-[#f9fafb]' => $selectedContactId !== $channel['user_id'],
                    ])>
                ...
                <span class="min-w-0 flex-1">
                    <span class="flex items-center justify-between gap-2">
                        <span class="truncate text-[13px] font-semibold text-[#1e1e1e]">{{ $channel['name'] }}</span>
                    ...
                    <span class="mt-0.5 flex items-center gap-1.5 text-[11px]">
                        <span class="text-[#9ca3af]">{{ $channel['role_label'] }}</span>
                        <span class="text-[#d1d5db]">·</span>
                        <span style="color: {{ $channel['online'] ? '#16a34a' : '#9ca3af' }}">{{ $channel['online'] ? 'Online' : 'Offline' }}</span>
                    </span>
    ...
    <div class="flex h-[820px] flex-1 flex-col overflow-hidden rounded-2xl border border-[#e5e7eb] bg-white">
        @if (! $selected)
            <div class="flex flex-1 flex-col items-center justify-center gap-2 text-center">
                ...
                <p class="text-[13px] text-[#6b7280]">Select a staff member to start messaging.</p>
            </div>
        @else
            @php($channel = collect($channels)->firstWhere('user_id', $selected->id))
            {{-- Header --}}
            <div class="flex items-center gap-3 border-b border-[#e5e7eb] px-5 py-4">
                ...
                <div>
                    <p class="text-[15px] font-semibold text-[#1e1e1e]">{{ $selected->name }}</p>
                    <p class="text-[11px]">
                        <span class="text-[#9ca3af]">{{ $channel['role_label'] ?? '' }}</span>
                        <span class="text-[#d1d5db]">·</span>
                        <span style="color: {{ $channel && $channel['online'] ? '#16a34a' : '#9ca3af' }}">{{ $channel && $channel['online'] ? 'Online' : 'Offline' }}</span>
                    </p>
                </div>
            </div>
                ...
                @forelse ($messages as $message)
                    <div class="flex {{ $message->sender_id === auth()->id() ? 'justify-end' : 'justify-start' }}">
                        <div @class([
                            ...
                            'bg-[#f38c00] text-white' => $message->sender_id === auth()->id(),
                            'bg-[#f9fafb] text-[#1e1e1e]' => $message->sender_id !== auth()->id(),
                        ])>
                            <p class="text-[13px] leading-relaxed">{{ $message->body }}</p>
                            <p @class([
                                'mt-1 text-[10px]',
                                'text-white/70' => $message->sender_id === auth()->id(),
                                'text-[#9ca3af]' => $message->sender_id !== auth()->id(),
            ...
            <form wire:submit.prevent="send" class="flex items-end gap-2 border-t border-[#e5e7eb] p-4">
                <textarea wire:model="body" rows="1" placeholder="Message {{ $selected->name }}..."
