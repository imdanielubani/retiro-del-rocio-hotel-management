...
<div class="flex flex-col gap-4" x-data="{ showFilters: @js($showFilters) }">
    ...
    <div class="flex flex-col gap-3 rounded-2xl border border-[#e5e7eb] bg-white px-5 py-4">
        <div class="flex flex-wrap items-center gap-3">
            <div class="relative w-full sm:w-[240px]">
                ...
                <input type="text" wire:model.live.debounce.300ms="search" placeholder="Search member, ID or plan…"
            ...
            <div class="ml-auto flex items-center gap-3">
                <p class="text-[11px] text-[#6b7280]"><span class="font-bold text-[#1e1e1e]">{{ $filteredCount }}</span> {{ \Illuminate\Support\Str::plural('member', $filteredCount) }}</p>
                @if ($hasFilters)
                ...
                <button type="button" wire:click="openCreate" class="flex h-[34px] shrink-0 items-center justify-center gap-1.5 rounded-lg bg-[#f38c00] px-4 text-[12px] font-bold text-white transition hover:bg-[#dd7f00]">
                    <svg class="size-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"><path d="M12 5v14M5 12h14"/></svg>Add Membership
                </button>
            </div>
        </div>
        <div x-show="showFilters" x-cloak x-collapse>
            <div class="flex flex-nowrap items-end gap-3 overflow-x-auto border-t border-[#e5e7eb] pt-4">
                <div class="flex min-w-[120px] flex-1 flex-col gap-1.5">
                    <label class="text-[11px] uppercase tracking-wide text-[#6b7280]">Plan</label>
                    <select wire:model.live="planFilter" class="h-9 rounded-lg border border-[#e5e7eb] bg-white px-2.5 text-[13px] focus:outline-none focus:ring-2 focus:ring-[#f38c00]/20">
                        <option value="">All Plans</option>
                        @foreach ($plans as $pl)<option value="{{ $pl->id }}">{{ $pl->name }}</option>@endforeach
                    </select>
                </div>
    ...
    <div class="overflow-hidden rounded-xl border border-[#e5e7eb] bg-white">
        @if ($memberships->isEmpty())
            <div class="flex flex-col items-center justify-center gap-3 py-16 text-center">
                <p class="text-[15px] font-semibold text-[#1e1e1e]">No memberships found</p>
                <p class="text-[13px] text-[#6b7280]">Gym subscriptions made on the website will appear here.</p>
            </div>
        @else
                ...
                <table class="w-full min-w-[1000px] border-collapse">
                    <thead>
                        <tr class="bg-[#f9fafb] text-left">
                            @foreach (['Membership ID','Member','Plan','Type','Amount','Payment','Status','Period','Action'] as $col)
                    ...
                    <tbody>
                        @foreach ($memberships as $m)
                            @php [$sC,$sB]=$m->statusColors(); [$pC,$pB]=$m->paymentColors(); @endphp
                            <tr wire:key="gm-{{ $m->id }}" class="border-b border-[#e5e7eb] {{ $loop->even ? 'bg-[#f9fafb]' : 'bg-white' }}">
                                <td class="px-4 py-3.5 text-[13px] font-bold text-[#f38c00]">{{ $m->code }}</td>
                                ...
                                <td class="px-4 py-3.5 text-right">@include('admin.gym.partials.membership-menu', ['m' => $m])</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            ...
            <div class="flex flex-col divide-y divide-[#e5e7eb] md:hidden">
                @foreach ($memberships as $m)
                    @php [$sC,$sB]=$m->statusColors(); [$pC,$pB]=$m->paymentColors(); @endphp
                    <div wire:key="gm-m-{{ $m->id }}" class="flex flex-col gap-2 px-4 py-3.5">
                        <div class="flex items-center justify-between">
                            <span class="text-[13px] font-bold text-[#f38c00]">{{ $m->code }}</span>
                            <div class="flex items-center gap-1.5">
                                <span class="rounded-full px-2 py-0.5 text-[10px] font-semibold" style="background: {{ $pB }}; color: {{ $pC }};">{{ $m->paymentLabel() }}</span>
            ...
            <div class="flex flex-col gap-3 border-t border-[#e5e7eb] px-5 py-3.5 sm:flex-row sm:items-center sm:justify-between">
                <p class="text-[12px] text-[#6b7280]">Showing {{ $memberships->count() }} of {{ number_format($memberships->total()) }} members</p>
                @if ($memberships->hasPages())
                    @php $last=$memberships->lastPage();$cur=$memberships->currentPage();$start=max(1,min($cur-1,$last-2));$end=min($last,$start+2); @endphp
                    <div class="flex items-center gap-2">
                        <button type="button" wire:click="previousPage" @disabled($memberships->onFirstPage()) class="flex size-8 items-center justify-center rounded-md border border-[#e5e7eb] bg-white text-[#6b7280] transition hover:bg-[#f9fafb] disabled:opacity-40"><svg class="size-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg></button>
                        ...
                        @endfor
                        <button type="button" wire:click="nextPage" @disabled(! $memberships->hasMorePages()) class="flex size-8 items-center justify-center rounded-md border border-[#e5e7eb] bg-white text-[#6b7280] transition hover:bg-[#f9fafb] disabled:opacity-40"><svg class="size-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg></button>
                    </div>
                @endif
            </div>
        @endif
    ...
    @include('admin.gym.partials.membership-detail')
    @include('admin.gym.partials.membership-create')
</div>
