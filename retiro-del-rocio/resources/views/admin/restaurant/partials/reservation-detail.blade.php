{{-- Reservation detail — centered modal. --}}
@if ($showDetail && $selected)
    @php [$sC,$sB]=$selected->statusColors(); [$pC,$pB]=$selected->paymentColors(); @endphp
    <div class="fixed inset-0 z-[95] flex items-center justify-center p-4" wire:key="rr-detail-{{ $selected->id }}">
        ...
        <div class="relative z-10 flex max-h-[90vh] w-full max-w-[480px] flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
            <div class="flex items-start justify-between border-b border-[#e5e7eb] px-6 py-5">
                <div>
                    ...
                    <div class="mt-2 flex flex-wrap items-center gap-2">
                        ...
                        <span class="inline-flex rounded-full bg-[#eef2f6] px-2.5 py-1 text-[11px] font-semibold text-[#475569]">{{ $selected->areaLabel() }}</span>
                    </div>
                </div>
            ...
            <div class="flex-1 overflow-y-auto px-6 py-5">
                ...
                <div class="mt-5 grid grid-cols-2 gap-3">
                    <div class="rounded-xl border border-[#e5e7eb] bg-[#f9fafb] px-4 py-3"><p class="text-[11px] uppercase tracking-wide text-[#9ca3af]">Occasion</p><p class="mt-0.5 text-[13px] font-semibold text-[#1e1e1e]">{{ $selected->occasion ?: '—' }}</p></div>
                ...
                @if ($selected->special_request)
                    <div class="mt-3 rounded-xl border border-[#e5e7eb] bg-[#f9fafb] px-4 py-3">
                        <p class="text-[11px] uppercase tracking-wide text-[#9ca3af]">Special request</p>
                        <p class="mt-1 text-[13px] leading-relaxed text-[#374151]">{{ $selected->special_request }}</p>
                    </div>
                @endif
            </div>
            <div class="flex flex-wrap items-center gap-2 border-t border-[#e5e7eb] px-6 py-4">
                <a href="{{ route('admin.restaurant.receipt', $selected->id) }}" target="_blank" class="flex-1 rounded-lg border border-[#e5e7eb] px-4 py-2.5 text-center text-[13px] font-semibold text-[#374151] transition hover:bg-[#f9fafb]">Receipt</a>
                @if (in_array($selected->status, ['confirmed', 'seated'], true))
                    <button type="button" wire:click="setStatus({{ $selected->id }}, 'completed')" class="flex-1 rounded-lg bg-[#dcfce7] px-4 py-2.5 text-[13px] font-bold text-[#16a34a] transition hover:bg-[#bbf7d0]">Complete</button>
                @endif
                @if (! in_array($selected->status, ['cancelled', 'completed'], true))
                    <button type="button" wire:click="cancelReservation({{ $selected->id }})" class="flex-1 rounded-lg bg-[#fee2e2] px-4 py-2.5 text-[13px] font-bold text-[#dc2626] transition hover:bg-[#fecaca]">Cancel</button>
                @endif
            </div>
        </div>
    </div>
