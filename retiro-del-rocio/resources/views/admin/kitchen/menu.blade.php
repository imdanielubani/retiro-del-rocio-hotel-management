<div class="flex flex-col gap-4">
    ...
    <div class="flex flex-col gap-3 rounded-2xl border border-[#e5e7eb] bg-white p-3.5 xl:flex-row xl:items-center xl:gap-3">
        <div class="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center sm:gap-2.5">
            ...
            <div class="flex flex-wrap items-center gap-1.5">
                <button type="button" wire:click="$set('category', '')" @class(['rounded-lg border px-3 py-1.5 text-[12px] font-medium transition', 'border-[#f38c00] bg-[#f38c00] font-semibold text-white' => $category === '', 'border-[#e5e7eb] bg-white text-[#6b7280] hover:bg-[#f9fafb]' => $category !== ''])>All</button>
        ...
        <div class="flex items-center justify-between gap-3 xl:shrink-0">
            <p class="shrink-0 whitespace-nowrap text-[13px] text-[#6b7280]"><span class="font-semibold text-[#1e1e1e]">{{ $items->count() }}</span> {{ \Illuminate\Support\Str::plural('dish', $items->count()) }}</p>
    ...
    @if ($items->isEmpty())
        <div class="flex flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-[#d6d9d2] bg-white py-16 text-center">
            <p class="text-[15px] font-semibold text-[#1e1e1e]">No dishes yet</p>
            <p class="text-[13px] text-[#6b7280]">Add a dish — it will appear on the guest tablet's Place Order menu.</p>
            <button type="button" wire:click="openCreate" class="mt-1 rounded-xl bg-[#f38c00] px-4 py-2 text-[13px] font-bold text-white hover:bg-[#dd7f00]">Add Dish</button>
        </div>
    @else
        <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
            @foreach ($items as $item)
                <div wire:key="menu-item-{{ $item->id }}" class="relative flex flex-col gap-2.5 rounded-2xl border border-[#e5e7eb] bg-white p-4 text-center">
                    <div class="absolute right-3 top-3">@include('admin.kitchen.partials.menu-item-menu', ['item' => $item])</div>
                    <div class="mx-auto flex h-24 w-24 items-center justify-center overflow-hidden rounded-xl bg-[#f9fafb]">
                        @if ($item->imageUrl())
                            <img src="{{ $item->imageUrl() }}" alt="{{ $item->name }}" class="h-full w-full object-cover">
                        @else
                            <svg class="size-8 text-[#d6d9d2]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M4 3h16v18l-8-4-8 4z"/></svg>
                        @endif
                    </div>
        ...
        <div class="fixed inset-0 z-[95] flex items-center justify-center p-4">
            ...
            <form wire:submit="save" class="relative z-10 my-auto w-full max-w-[460px] overflow-hidden rounded-2xl bg-white shadow-xl">
                <div class="flex items-center justify-between border-b border-[#e5e7eb] px-6 py-4">
                    <h3 class="text-[17px] font-bold text-[#1e1e1e]">{{ $editingId ? 'Edit Dish' : 'Add Dish' }}</h3>
                ...
                <div class="flex max-h-[70vh] flex-col gap-4 overflow-y-auto px-6 py-5">
                    <div class="flex flex-col gap-1.5">
                        <label class="text-[12px] font-semibold text-[#374151]">Dish name</label>
                        <input type="text" wire:model="fName" placeholder="e.g. Pan-Seared Salmon" class="h-11 rounded-xl border border-[#e5e7eb] px-3.5 text-[14px] focus:border-[#f38c00] focus:outline-none focus:ring-2 focus:ring-[#f38c00]/15">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div class="flex flex-col gap-1.5">
                            ...
                            <input type="number" min="0" wire:model="fPrice" placeholder="8500" class="h-11 rounded-xl border border-[#e5e7eb] px-3.5 text-[14px] focus:border-[#f38c00] focus:outline-none focus:ring-2 focus:ring-[#f38c00]/15">
                        ...
                        <div class="flex flex-col gap-1.5">
                            ...
                            <input type="number" min="0" wire:model="fPrepMinutes" placeholder="25" class="h-11 rounded-xl border border-[#e5e7eb] px-3.5 text-[14px] focus:border-[#f38c00] focus:outline-none focus:ring-2 focus:ring-[#f38c00]/15">
                ...
                <div class="flex justify-end gap-2 border-t border-[#e5e7eb] px-6 py-4">
                    ...
                    <button type="submit" class="flex items-center gap-2 rounded-xl bg-[#f38c00] px-5 py-2.5 text-[14px] font-bold text-white transition hover:bg-[#dd7f00]">
                        <span wire:loading.remove wire:target="save">{{ $editingId ? 'Save changes' : 'Add dish' }}</span>
    ...
    @include('admin.kitchen.partials.category-manager')
</div>
