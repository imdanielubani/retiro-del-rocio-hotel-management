<div class="flex flex-col gap-4">
    ...
    <div class="flex flex-wrap items-center gap-3 rounded-2xl border border-[#e5e7eb] bg-white px-5 py-4">
        ...
        <p class="ml-auto text-[11px] text-[#6b7280]">A ticket counts once it's been assigned to a chef and served.</p>
    </div>
    ...
    <div class="overflow-hidden rounded-xl border border-[#e5e7eb] bg-white">
        @if ($staff->isEmpty())
            <div class="flex flex-col items-center justify-center gap-3 py-16 text-center">
                <p class="text-[15px] font-semibold text-[#1e1e1e]">No kitchen staff yet</p>
                <p class="text-[13px] text-[#6b7280]">Create staff accounts with the Kitchen role in Access &amp; Users.</p>
            </div>
        @else
                ...
                <table class="w-full min-w-[640px] border-collapse">
                    <thead>
                        <tr class="bg-[#f9fafb] text-left">
                            @foreach (['Staff','Status','Active Tickets','Tickets Served','Value Served'] as $col)
                    ...
                    <tbody>
                        @foreach ($staff as $row)
                            <tr class="border-b border-[#f3f4f6] last:border-0">
                                ...
                                <td class="px-4 py-3 text-[13px] text-[#1e1e1e]">{{ $row['active_tickets'] }}</td>
                                <td class="px-4 py-3 text-[13px] font-semibold text-[#1e1e1e]">{{ $row['tickets_served'] }}</td>
                                <td class="px-4 py-3 text-[13px] text-[#6b7280]">{{ $row['revenue_label'] }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
