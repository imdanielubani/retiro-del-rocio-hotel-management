{{-- Kitchen menu item row action menu (fixed positioning). $item = MenuItem. --}}
<div x-data="{ open: false, pos: { top: '0px', left: '0px' } }" class="relative inline-block text-left">
    ...
    <div x-show="open" x-cloak @click.outside="open = false" x-transition.opacity.duration.100ms
        ...
        <button type="button" @click="open = false" wire:click="edit({{ $item->id }})" class="flex w-full items-center gap-3 px-4 py-2.5 text-left text-[14px] font-medium text-[#1e1e1e] transition hover:bg-[#f9fafb]">
        ...
        <button type="button" @click="open = false" wire:click="toggleActive({{ $item->id }})" class="flex w-full items-center gap-3 px-4 py-2.5 text-left text-[14px] font-medium text-[#374151] transition hover:bg-[#f9fafb]">
            ...
            {{ $item->is_active ? 'Hide from menu' : 'Show on menu' }}
        </button>
        <div class="h-px bg-[#f1f1ee]"></div>
        <button type="button" @click="open = false" wire:click="delete({{ $item->id }})" wire:confirm="Delete {{ $item->name }}?" class="flex w-full items-center gap-3 px-4 py-2.5 text-left text-[14px] font-medium text-[#dc2626] transition hover:bg-[#fef2f2]">
