{{-- Kitchen "Manage Categories" modal. $categories = food MenuCategory collection. --}}
    ...
    <div class="fixed inset-0 z-[95] flex items-center justify-center p-4">
        ...
        <div class="relative z-10 my-auto w-full max-w-[420px] overflow-hidden rounded-2xl bg-white shadow-xl">
            <div class="flex items-center justify-between border-b border-[#e5e7eb] px-6 py-4">
                <h3 class="text-[17px] font-bold text-[#1e1e1e]">Kitchen Categories</h3>
            ...
            <div class="flex flex-col gap-4 px-6 py-5">
                <form wire:submit="addCategory" class="flex items-start gap-2">
                    <div class="flex-1">
                        <input type="text" wire:model="newCategoryName" placeholder="e.g. Grills" class="h-11 w-full rounded-xl border border-[#e5e7eb] px-3.5 text-[14px] focus:border-[#f38c00] focus:outline-none focus:ring-2 focus:ring-[#f38c00]/15">
                ...
                <div class="flex flex-col divide-y divide-[#f1f1ee] rounded-xl border border-[#e5e7eb]">
                    @forelse ($categories as $c)
                        <div wire:key="kitchen-cat-{{ $c->id }}" class="flex items-center justify-between px-3.5 py-2.5">
                            <div>
                                <p class="text-[13px] font-medium text-[#1e1e1e]">{{ $c->name }}</p>
                                <p class="text-[11px] text-[#9ca3af]">{{ $c->itemCount() }} {{ \Illuminate\Support\Str::plural('dish', $c->itemCount()) }}</p>
                            </div>
