...
<head>
    ...
    <title>Receipt {{ $m->code }} — {{ config('app.name') }}</title>
    <style>
...
<body onload="window.print()">
    <div class="sheet">
        <div class="head">
            ...
            <span class="badge">{{ $m->paymentLabel() }}</span>
        </div>
        <div class="body">
            <p style="margin:0 0 4px;color:#6b7280;font-size:13px;text-transform:uppercase;letter-spacing:.5px;">Membership receipt</p>
            <div class="code">{{ $m->code }}</div>
            <table>
                <tr><td class="k">Member</td><td class="v">{{ $m->customer_name ?: '—' }}</td></tr>
                <tr><td class="k">Email</td><td class="v">{{ $m->customer_email ?: '—' }}</td></tr>
                @if ($m->customer_phone)<tr><td class="k">Phone</td><td class="v">{{ $m->customer_phone }}</td></tr>@endif
                <tr><td class="k">Plan</td><td class="v">{{ $m->plan_name }}</td></tr>
                <tr><td class="k">Type</td><td class="v">{{ $m->typeLabel() }}</td></tr>
                <tr><td class="k">Start date</td><td class="v">{{ optional($m->starts_at)->format('M j, Y') ?: '—' }}</td></tr>
